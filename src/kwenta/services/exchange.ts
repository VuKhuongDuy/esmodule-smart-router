...
import erc20Abi from '../contracts/abis/ERC20.json' assert { type: "json" }
...
export default class ExchangeService {
	...
	public async getOneInchTokenList() {
		...
		return {
			tokens,
			tokensMap: lodash.keyBy(tokens, 'symbol'),
	...
	public async getOneInchQuote(baseCurrencyKey: string, quoteCurrencyKey: string, amount: string) {
		...
		if (this.tokensMap[quoteCurrencyKey] && synthUsdRate) {
			const usdAmount = wei(amount).div(synthUsdRate[0])
			...
			return wei(estimatedAmount).mul(synthUsdRate ? synthUsdRate[0] : 0).toString()
		}
	}
	...
	private getTokenDecimals(currencyKey: string) {
		return lodash.get(this.allTokensMap, [currencyKey, 'decimals'], undefined)
	}
	...
	private getTokenAddress(currencyKey: string, coingecko?: boolean) {
		if (currencyKey != null) {
			if (this.isCurrencyETH(currencyKey)) {
				...
				return lodash.get(this.allTokensMap, [currencyKey, 'address'], null)
			}
		} else {
			return null
		}
	...
	public async getTokenBalances(walletAddress: string): Promise<TokenBalances> {
		...
		const tokensMap = lodash.keyBy(filteredTokens, 'symbol')
		const calls = []
