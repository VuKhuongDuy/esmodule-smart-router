...
import PerpsMarketABI from '../contracts/abis/PerpsV2Market.json' assert { type: "json" }
import SmartMarginAccountABI from '../contracts/abis/SmartMarginAccount.json' assert { type: "json" }
...
export default class FuturesService {
	...
	public getSkewAdjustedPrice = async (price: Wei, marketAddress: string, marketKey: string) => {
		...
		const skewWei = wei(marketSkew as WeiSource)
		const scaleWei = wei(skewScale as WeiSource)
